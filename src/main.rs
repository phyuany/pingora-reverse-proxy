...
pub fn main() {
    ...
    let proxy_service_ssl = proxy_service_tls(
        ...
        vec![
            HostConfigTls {
                proxy_addr: "10.0.0.9:80".to_owned(),
                proxy_tls: false,
                proxy_hostname: "mp.xscat.net:8088".to_owned(),
                cert_path: format!("{}/keys/xscat.net.crt", env!("CARGO_MANIFEST_DIR")),
                key_path: format!("{}/keys/xscat.net.key", env!("CARGO_MANIFEST_DIR")),
            },
            HostConfigTls {
                proxy_addr: "10.0.0.9:443".to_owned(),
                proxy_tls: true,
                proxy_hostname: "mp.xscat.net".to_owned(),
                cert_path: format!("{}/keys/xscat.net.crt", env!("CARGO_MANIFEST_DIR")),
                key_path: format!("{}/keys/xscat.net.key", env!("CARGO_MANIFEST_DIR")),
            },
        ],
    ...
    // 配置HTTP重定向服务
    let http_redirect_app = new_http_redirect_app(&format!("0.0.0.0:8080"));
